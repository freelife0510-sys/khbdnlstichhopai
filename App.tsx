...
const App: React.FC = () => {
  ...
  const themeColor = isIntegrationMode ? 'teal' : isAIMode ? 'purple' : 'blue';
  ...
  const bgColor = isIntegrationMode ? 'bg-[#E0F2F1]' : isAIMode ? 'bg-[#F3E5F5]' : 'bg-[#E3F2FD]';

  return (
    <div className={`min-h-screen font-sans pb-12 ${bgColor}`}>
      <Header
      ...
      <main className="max-w-5xl mx-auto px-4 mt-8">
        {/* Integration Mode - Separate Tab Content */}
        {isIntegrationMode ? (
      ...
      <footer className={`mt-12 text-center ${isIntegrationMode ? 'text-teal-800/60' : isAIMode ? 'text-purple-800/60' : 'text-blue-800/60'} text-sm py-6`}>
        <p>© 2024 NLS & AI Education Assistant. Built with Gemini API & React.</p>
        <p className={`mt-3 font-medium ${isIntegrationMode ? 'text-teal-800' : isAIMode ? 'text-purple-800' : 'text-blue-800'}`}>
