    ...
    >
      ...
      {isProcessing ? (
        <div className="flex flex-col items-center animate-pulse py-2">
          <Loader2 className={`${isAIMode ? 'text-purple-600' : 'text-blue-600'} animate-spin mb-2`} size={32} />
          <p className={`text-sm font-medium ${isAIMode ? 'text-purple-900' : 'text-blue-900'}`}>Đang đọc file...</p>
        </div>
      ) : hasContent ? (
        ...
        <div className="flex flex-col items-center py-2">
          <div className="p-3 bg-white rounded-full shadow-sm mb-3 group-hover:scale-110 transition-transform">
            {isLesson
              ? <FileText className={isAIMode ? 'text-purple-500' : 'text-blue-500'} size={28} />
              : <FileUp className={isAIMode ? 'text-purple-500' : 'text-indigo-500'} size={28} />
            }
          </div>
          <p className="text-base font-semibold text-slate-800">{title}</p>
          <p className="text-sm text-slate-500 mt-1">{subTitle}</p>
          <p className={`text-xs mt-3 bg-white px-2 py-1 rounded border ${isAIMode ? 'text-purple-400 border-purple-100' : 'text-blue-400 border-blue-100'}`}>
            Hỗ trợ .docx, .pdf
          </p>
        </div>
      )}
  ...
  return (
    <div className={`bg-white p-6 rounded-xl shadow-sm border ${isAIMode ? 'border-purple-100' : 'border-blue-100'} mb-6`}>
      <div className="flex items-center mb-6">
        ...
        <h2 className={`text-lg font-semibold ${isAIMode ? 'text-purple-900' : 'text-blue-900'}`}>Tài liệu đầu vào</h2>
      </div>

      <div className={`grid grid-cols-1 ${isAIMode ? '' : 'md:grid-cols-2'} gap-6`}>
        {/* Ô Upload Giáo án */}
        ...
        {/* Ô Upload PPCT - chỉ hiển thị ở mode NLS */}
        {!isAIMode && (
          <div className="space-y-2">
